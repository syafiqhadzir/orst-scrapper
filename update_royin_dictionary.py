...
from scripts.config import DEFAULT_SCRAPER_CONFIG, ScraperConfig
...
def main() -> int:
    ...
    try:
        ...
        if CURRENT_DIC_FILE.exists():
            ...
            logger.info(f"✓ Loaded {len(old_words)} words from {CURRENT_DIC_FILE.name}\n")
        else:
        ...
        if args.dry_run:
            ...
            if CURRENT_DIC_FILE.exists() and not args.no_backup:
                ...
                backup_path = CURRENT_DIC_FILE.parent / f"th_TH-royin.{timestamp}.backup.dic"
